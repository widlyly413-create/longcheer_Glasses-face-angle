...
def process_image_v28(image_path):
    ...
    if best_set is None:
        return img, 0, "识别失败：多级级联区内未匹配到合规眼镜刚体（轻微虚化或遮挡可能影响了检测）"
...
def main():
    parser = argparse.ArgumentParser(description='面弯角精密测量系统 (V28 增强容错版 - 处理轻微虚化和遮挡)')
    parser.add_argument('input', help='输入图片路径或包含图片的文件夹')
    parser.add_argument('-o', '--output', default='output_v28', help='输出文件夹路径')
    ...
    for img_path in image_paths:
        ...
        res_img, angle, status = process_image_v28(img_path)
