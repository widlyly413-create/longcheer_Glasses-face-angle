...
# --- 核心算法层：包容金属砝码 + 硬掩膜锁死 + 拓扑几何检测 ---
def process_image_v21(image_bytes):
    ...
    # ==========================================
    # 步骤 1：包容性提取（黑框 + 银色金属砝码）
    # ==========================================
    # 【核心修改 1】：将 V 通道上限提升至 150，将亮银色金属配重也作为“框架”的一部分提取
    lower_frame, upper_frame = np.array([0, 0, 0]), np.array([180, 255, 150]) 
    ...
    # 【核心修改 2】：使用超大膨胀核(45x45)，强行搭桥缝合金属反光造成的物理断裂，并包容侧视导致的红点溢出
    kernel_dilate = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (45, 45))
...
# --- Streamlit 现代 UI 用户层 ---
st.set_page_config(page_title="WrapAngle V21", layout="wide")
st.title("👓 面弯角测量系统 (V21 金属砝码包容版)")
...
with tab1:
    ...
    if single_file:
        res_img, ang, status = process_image_v21(single_file.read())
    ...
    if zip_file:
        ...
        with zipfile.ZipFile(zip_file, "r") as z_in, zipfile.ZipFile(out_zip, "w") as z_out:
            ...
            for i, f_name in enumerate(files):
                res_img, ang, status = process_image_v21(z_in.read(f_name))
        ...
        st.success("批量数据解析完毕！")
        st.download_button("📥 导出标注图片包 (Zip)", out_zip.getvalue(), "Measurement_Results_V21.zip")
...
if st.session_state.history:
    ...
    st.download_button("📊 导出报表 (CSV)", df_h.to_csv(index=False).encode('utf-8-sig'), "data_history_v21.csv", "text/csv")
